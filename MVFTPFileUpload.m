...
//											   kCFBooleanTrue);
		...
		if ( success == false ) {
			if ( [self.delegate respondsToSelector:@selector(fileUpload:didFailWithError:)] ) {
				[self.delegate fileUpload:self didFailWithError:@"CFWriteStreamOpen failed"];
			}
		} else {
		...
		if ( [self.delegate respondsToSelector:@selector(fileUpload:didFailWithError:)] ) {
			[self.delegate fileUpload:self didFailWithError:@"CFWriteStreamSetClient failed"];
		}
		fprintf(stderr, "CFWriteStreamSetClient failed\n");
	}
	} else {
		if ( [self.delegate respondsToSelector:@selector(fileUpload:didFailWithError:)] ) {
			[self.delegate fileUpload:self didFailWithError:@"CFReadStreamOpen failed"];
		}
	...
	switch ( type ) {
		...
		case kCFStreamEventCanAcceptBytes:
			if ( self.leftOverByteCount > 0 ) {
				...
				if ( bytesRead < 0 ) {
					if ( [self.delegate respondsToSelector:@selector(fileUpload:didFailWithError:)] ) {
						[self.delegate fileUpload:self didFailWithError:@"CFReadStreamRead"];
					}
					return;
				}
			...
			if ( bytesAvailable == 0 ) {
				...
				if ( bytesWritten > 0 ) {
					...
					if ( [self.delegate respondsToSelector:@selector(fileUpload:didFailWithError:)] ) {
						[self.delegate fileUpload:self didFailWithError:@"CFWriteStreamWrite"];
					}
				}
			}
